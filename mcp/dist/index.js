...
function errorResult(err) {
    const parsed = (0, core_1.parseError)(err);
    return {
        content: [{ type: "text", text: `Error: ${parsed.message}` }],
        isError: true,
    };
}
...
// ─── atlas_create_world ───────────────────────────────────────────────────
server.tool("atlas_create_world", "Crea un nuevo mundo persistente en Atlas World Protocol (Solana). " +
    ...
    name: zod_1.z.string().max(64).describe("Nombre del mundo"),
    worldType: zod_1.z.enum(["gaming", "dao", "marketplace", "custom"]).describe("Narrativa del mundo — no cambia el comportamiento del contrato, solo documenta el propósito"),
    visibility: zod_1.z.enum(["public", "private"]).default("public").describe("public: cualquier wallet puede participar, gratis. private: requiere whitelist, cobra fee de creación"),
    totalResources: zod_1.z.number().int().positive().default(500).describe("Recursos totales disponibles por epoch — cuando se agotan, el mundo entra en WorldReset"),
    epochDuration: zod_1.z.string().default("7d").describe("Duración máxima del epoch: '7d', '24h', '30m', o segundos como número"),
    globalCooldown: zod_1.z.number().int().nonnegative().default(5).describe("Segundos mínimos entre recolectas por wallet (anti-bot)"),
    maxDailyCollects: zod_1.z.number().int().nonnegative().default(0).describe("Límite de recolectas por wallet cada 24h. 0 = sin límite"),
    resourceTypes: zod_1.z
        .array(zod_1.z.object({
        id: zod_1.z.number().int().min(0).max(7),
        name: zod_1.z.string().max(32),
        points: zod_1.z.number().int().positive(),
        cooldownSeconds: zod_1.z.number().int().nonnegative(),
    }))
        .min(1)
        .max(8)
    ...
    try {
        ...
        const { worldId, signature } = await atlas.world.create({
            ...
            visibility: args.visibility === "private" ? core_1.WorldVisibility.Private : core_1.WorldVisibility.Public,
        ...
        return textResult(`Mundo creado exitosamente.\nworldId: ${worldId}\nsignature: ${signature}\n\n` +
...
// ─── atlas_mint_player ─────────────────────────────────────────────────────
server.tool("atlas_mint_player", "Mintea un Player (identidad on-chain) para la wallet configurada, en un " +
    ...
    worldId: zod_1.z.number().int().nonnegative().describe("ID del mundo (usa el default si se omite)").optional(),
    name: zod_1.z.string().max(32).describe("Nombre del player"),
}, async (args) => {
    try {
        const atlas = (0, client_1.getAtlasClient)();
        const worldId = args.worldId ?? (0, client_1.getDefaultWorldId)();
        ...
        if (alreadyExists) {
            ...
            return textResult(`Esta wallet ya tiene un player en el mundo ${worldId}: "${player?.name}", ` +
...
server.tool("atlas_collect_resource", "Recolecta un recurso del mundo compartido. Verifica on-chain el cooldown, " +
    ...
    worldId: zod_1.z.number().int().nonnegative().optional().describe("ID del mundo (usa el default si se omite)"),
    resourceTypeId: zod_1.z.number().int().min(0).max(7).default(0).describe("ID del tipo de recurso a recolectar, según los resourceTypes definidos al crear el mundo"),
}, async (args) => {
    try {
        const atlas = (0, client_1.getAtlasClient)();
        const worldId = args.worldId ?? (0, client_1.getDefaultWorldId)();
        ...
        let text = `Recolectado: +${result.points} puntos (${result.resourceType.name}). ` +
...
server.tool("atlas_get_world_status", "Obtiene el estado completo de un mundo: nombre, visibilidad, epoch actual, " +
...
server.tool("atlas_get_player_status", "Obtiene el estado del Player de la wallet configurada en un mundo: " +
...
server.tool("atlas_get_leaderboard", "Obtiene el leaderboard (top 25) de un mundo, para el epoch actual o uno " +
    "específico. El leaderboard de epochs pasados persiste como historial.", {
    worldId: zod_1.z.number().int().nonnegative().optional().describe("ID del mundo (usa el default si se omite)"),
    epoch: zod_1.z.number().int().nonnegative().optional().describe("Epoch específico — si se omite, usa el actual"),
}, async (args) => {
    try {
        const atlas = (0, client_1.getAtlasClient)();
        const worldId = args.worldId ?? (0, client_1.getDefaultWorldId)();
...
// ─── atlas_advance_epoch ─────────────────────────────────────────────────────
server.tool("atlas_advance_epoch", "Avanza un mundo al siguiente epoch después de que se agotó (WorldReset). " +
