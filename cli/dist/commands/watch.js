...
async function watchCommand(options) {
    ...
    console.log(chalk_1.default.gray(`Revisando cada ${POLL_INTERVAL_MS / 1000}s. Cuando el mundo se agote, avanzo el epoch solo.\n`));
    let stopped = false;
    let lastEpochSeen = null;
    process.on("SIGINT", () => {
        stopped = true;
    ...
    while (!stopped) {
        try {
            ...
            if (!world) {
                console.log(chalk_1.default.red(`❌ Mundo ${worldId} no encontrado. Deteniendo.\n`));
                break;
            }
            if (lastEpochSeen === null) {
                lastEpochSeen = world.config.currentEpoch;
                console.log(chalk_1.default.gray(`   Epoch actual: ${world.config.currentEpoch} · progreso: ${world.progress}%`));
            }
            ...
            if (world.pendingAdvance) {
                ...
                lastEpochSeen = world.config.currentEpoch;
                console.log(chalk_1.default.green(`✅ Epoch ${lastEpochSeen} listo — el mundo sigue aceptando recolectas\n`));
            }
            ...
            console.log(chalk_1.default.red(`⚠️  Error revisando el mundo: ${err.message ?? err}`));
        }
